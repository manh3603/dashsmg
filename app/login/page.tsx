...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (!isBackendConfigured()) {
      queueMicrotask(() => setApiReachable(false));
      return;
    }
    let cancelled = false;
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10 border border-slate-200">
          <form className="space-y-6" onSubmit={(e) => void handleLogin(e)}>
            ...
            <div>
              ...
              <input
                ...
                className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 text-slate-900"
                value={login}
                onChange={(e) => setLogin(e.target.value)}
              />
              <p className="mt-1 text-xs text-slate-500">Dùng email đã đăng ký hoặc tài khoản do quản trị cấp.</p>
            </div>
            <div>
              ...
              <div className="relative mt-1">
                <input
                  ...
                  className="block w-full rounded-md border border-slate-300 py-2 pl-3 pr-11 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500 text-slate-900"
                  value={password}
          ...
          <div className="mt-6 rounded-lg border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Kết nối &amp; hỗ trợ</p>
            {!isBackendConfigured() && (
            ...
            {hints && hints.length > 0 && (
              <>
                <p className="mt-3 text-xs font-medium text-slate-600">Gợi ý tài khoản phát triển (AUTH_DEMO_HINTS=1)</p>
